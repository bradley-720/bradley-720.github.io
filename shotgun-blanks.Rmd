...
NMDS relies on optimization, so you probably want to pay attention to whether they converged. PCA and PCoA have exact solutions, so this isn't a problem, but you may want to check how much variance each dimension explains. Very big numbers (e.g. 80\%) and very small numbers (e.g. 5\%) can be red flags that there are batch effects or low signal, respectively. For PCA and PCoA you always get back as many dimensions as the number of samples or the number of genes, whichever is smaller -- for NMDS and t-SNE you can specify the number of dimensions you want to optimize for. PCA and PCoA are very common in microbial ecology, with NMDS a little less common. UMAP is most common in the analysis of single-cell RNA-seq data.
...
mgs_umap <- umap(mtx_reads_mgs %>% t, neighbors = 20)
mgs_palette = palette()[c(3, 2, 4)]
mgs_colors = mgs_palette[mgs.metadata$health_state]
plot(mgs_umap[, 1], mgs_umap[, 2], col = mgs_colors,
  bg = mgs_colors, pch = 21, main = "t-SNE")
ordiellipse(mgs_umap, mgs_metadata$health_state, col = mgs_palette,
